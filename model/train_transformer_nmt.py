...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        q = self.q_proj(q).view(B, Tq, self.n_heads, self.head_dim).transpose(1, 2)  # [B,H,Tq,hd]
        k = self.k_proj(k).view(B, Tk, self.n_heads, self.head_dim).transpose(1, 2)  # [B,H,Tk,hd]
        v = self.v_proj(v).view(B, Tk, self.n_heads, self.head_dim).transpose(1, 2)  # [B,H,Tk,hd]
        ...
        if attn_mask is not None:
            scores = scores + attn_mask  # attn_mask: broadcastable to [B,H,Tq,Tk]
...
class TransformerNMT(nn.Module):
    def __init__(
        ...
        if pos_encoding == "sinusoidal":
            ...
            # RoPE is applied inside attention, no additive positional embedding
            pass
        else:
    ...
    def make_causal_mask(self, T: int, device, dtype) -> torch.Tensor:
        # Additive mask: -inf for disallowed positions
    ...
    def encode(self, src: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        # src: [B,S]
        src_pad_mask = self.make_padding_mask(src)  # [B,S]
        x = self.src_emb(src)
    ...
    def decode(self, tgt_in: torch.Tensor, memory: torch.Tensor, src_pad_mask: torch.Tensor) -> torch.Tensor:
        # tgt_in: [B,T]
        tgt_pad_mask = self.make_padding_mask(tgt_in)  # [B,T]
        ...
        T = tgt_in.size(1)
        causal = self.make_causal_mask(T, device=tgt_in.device, dtype=x.dtype)  # [T,T]
    ...
    def greedy_decode(self, src: torch.Tensor, bos_id: int, eos_id: int, max_len: int) -> List[List[int]]:
        ...
        for _ in range(max_len):
            logits = self.decode(ys, memory, src_pad_mask)  # [B,t,V]
...
def label_smoothed_nll_loss(logits: torch.Tensor, targets: torch.Tensor, pad_id: int, eps: float) -> torch.Tensor:
    ...
    nll = F.nll_loss(logp, targets, reduction="none", ignore_index=pad_id)  # [N]
    if eps <= 0.0:
        return nll.mean()

    # smooth loss: mean negative log prob over vocab
...
def main():
    ...
    # 自动修正 total_steps，使其与真实训练步数一致
    steps_per_epoch = len(train_loader)
    true_total_steps = steps_per_epoch * args.epochs

    # 如果用户没显式传 total_steps 或者传了明显不合理的值，则自动覆盖
    ...
    print(
        f"[schedule] steps_per_epoch={steps_per_epoch} total_steps={args.total_steps} warmup_steps={args.warmup_steps}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            with torch.cuda.amp.autocast(enabled=bool(args.fp16 and device.type == "cuda")):
                logits = model(batch.src, batch.tgt_in)  # [B,T,V]
