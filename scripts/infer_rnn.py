# scripts/infer_rnn.py
import argparse
import json
from pathlib import Path
from typing import Dict, List

import torch
from torch.utils.data import DataLoader

from nmt.data import JsonlIdsDataset, collate_ids
from nmt.models.decoding import greedy_decode, beam_search_decode_one
from nmt.models.rnn_nmt import RNNNMT, NMTConfig


def load_vocab(path: str) -> Dict[str, int]:
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)


def invert_vocab(vocab: Dict[str, int]) -> Dict[int, str]:
    return {i: t for t, i in vocab.items()}


def ids_to_sentence(ids: List[int], id2tok: Dict[int, str], eos_id: int) -> str:
    toks = []
    for i in ids:
        if i == eos_id:
            break
        tok = id2tok.get(i, "<unk>")
        # skip specials
        if tok in ("<pad>", "<bos>", "<eos>"):
            continue
        toks.append(tok)
    return " ".join(toks)


@torch.no_grad()
def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--ckpt", required=True, help="path to best.pt")
    ap.add_argument("--data_dir", required=True, help="word_tok dir containing test.ids.jsonl")
    ap.add_argument("--vocab_zh", required=True)
    ap.add_argument("--vocab_en", required=True)

    ap.add_argument("--decode", choices=["greedy", "beam"], default="greedy")
    ap.add_argument("--beam_size", type=int, default=5)
    ap.add_argument("--max_len", type=int, default=128)
    ap.add_argument("--batch_size", type=int, default=64)

    ap.add_argument("--out_hyp", required=True, help="output hypothesis txt file")
    ap.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    args = ap.parse_args()

    vocab_zh = load_vocab(args.vocab_zh)
    vocab_en = load_vocab(args.vocab_en)
    id2tok = invert_vocab(vocab_en)

    pad_id = vocab_zh.get("<pad>", 0)
    bos_id = vocab_en.get("<bos>", 1)
    eos_id = vocab_en.get("<eos>", 2)

    ckpt = torch.load(args.ckpt, map_location="cpu")
    cfg = NMTConfig(**ckpt["cfg"])
    device = torch.device(args.device)
    model = RNNNMT(cfg, pad_id=pad_id, bos_id=bos_id, eos_id=eos_id).to(device)
    model.load_state_dict(ckpt["model"])
    model.eval()

    test_ds = JsonlIdsDataset(str(Path(args.data_dir) / "test.ids.jsonl"))
    loader = DataLoader(
        test_ds, batch_size=args.batch_size, shuffle=False,
        collate_fn=lambda b: collate_ids(b, pad_id=pad_id),
        num_workers=2, pin_memory=True
    )

    out_path = Path(args.out_hyp)
    out_path.parent.mkdir(parents=True, exist_ok=True)

    with out_path.open("w", encoding="utf-8") as fout:
        if args.decode == "greedy":
            for batch in loader:
                src = batch.src_ids.to(device)
                lens = batch.src_lens.to(device)
                pred = greedy_decode(model, src, lens, max_len=args.max_len)  # [B,T]
                for row in pred.tolist():
                    fout.write(ids_to_sentence(row, id2tok, eos_id) + "\n")
        else:
            # beam: do one by one (simpler & safe), can optimize later
            for batch in loader:
                src = batch.src_ids.to(device)
                lens = batch.src_lens.to(device)
                for i in range(src.size(0)):
                    one = beam_search_decode_one(
                        model, src[i:i+1], lens[i:i+1],
                        beam_size=args.beam_size, max_len=args.max_len
                    )
                    fout.write(ids_to_sentence(one, id2tok, eos_id) + "\n")

    print("Saved hyp:", str(out_path))


if __name__ == "__main__":
    main()
